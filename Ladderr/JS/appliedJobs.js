...
{ 
    ...
    const form = document.getElementById("ApproveRejectForm").children;
    for(var child of form)
    {
        if(child.value == choseOption && child.tagName == "INPUT")
        {
            child.checked = true;
            break;
        }
...
function loaded(){
    ...
    {
        let myFormMan = document.getElementById("ApproveRejectForm");
        for(var i of myFormMan.children)
        {
            if(i.value == "All")
            {
                i.checked = true;
